document.addEventListener('DOMContentLoaded', function() {
    ...
    function createEmojiGrid() {
        emojiGrid.innerHTML = '';
        let emojiArray = Array(6).fill(currentTarget);
        
        while(emojiArray.length < 25) {
            const randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];
            if(randomEmoji !== currentTarget || Math.random() > 0.7) {
        ...
        if(emoji === currentTarget) {
            ...
            setTimeout(() => {
                ...
                if(score < targetScore) {
                    ...
                    nextLevelBtn.innerHTML = '<button onclick="window.location.href=\'nivel3.html\'">Siguiente Nivel</button>';
                }
            }, 300);
        } else {
